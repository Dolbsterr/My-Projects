...
describe('Section 1: Functional tests', () => {
    ...
    it('User can use only same both first and validation passwords', ()=>{
        ...
        cy.get('input[name="password"]').type('KertIsBatman')
        cy.get('[name="confirm"]').type('KertIsBatman')
        cy.get('#email').type('heydude@hotmail.com')
        cy.get('#username').type('Batman')
        cy.get('[data-cy="name"]').type('Kert')
        cy.get('#lastName').type('Batman');
    ...
    it('User cannot submit form with one mandatory field empty', () => {
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('input[name="password"]').type('KertIsBatman')
        cy.get('[name="confirm"]').type('KertIsBatman')
        cy.get('#username').type('Batman')
...
// Create similar test for checking the second picture
    it('Check that logo 2 is correct and correct size', () => {
        cy.log('Will check logo source and size')
        cy.get('[data-cy="cypress_logo"]')
        .should('be.visible') 
        .and('have.attr', 'src', 'cypress_logo.png') 
        .and('have.attr', 'alt', 'cerebrum_hub_logo') 
        .and('have.attr', 'width', '116') 
        .and('have.attr', 'height', '88'); 
    ...
    it('Check if checkboxes list are correct and work', () => {
        ...
        cy.get('#vehicle1').eq(0).should('not.be.checked')
        cy.get('#vehicle2').eq(0).should('not.be.checked')
        cy.get('#vehicle3').eq(0).should('not.be.checked')

        cy.get('#vehicle1').eq(0).check().should('be.checked')
        cy.get('#vehicle2').eq(0).check().should('be.checked')
        cy.get('#vehicle3').eq(0).should('not.be.checked')
...
function inputValidData(Kert) {
    cy.log('Username will be filled')
    cy.get('input[data-testid="user"]').type(Kert)
    cy.get('#email').type('heydudehotmail.com')
    cy.get('[data-cy="name"]').type('Kert')
    cy.get('#lastName').type('Batman')
    cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
